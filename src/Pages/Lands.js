...
function Lands() {
  ...
  useEffect(() => {
      ...
      }, [selectedCity, selectedOwner, sortOrder, Lands]);
    
      const uniqueCities = [...new Set(Lands.map(apt => apt.city))].filter(Boolean);
      const uniqueOwners = [...new Set(Lands.map(apt => apt.owner))].filter(Boolean);
  ...
  <div className="card apartment-card shadow-sm border-0">
    ...
    <div className="apartment-body bg-secondary">
      <ul>
        ...
        {apt.number && (<li className="yellow fs-4"><strong className="text-light fs-5">رقم الأرض :</strong> {apt.number}</li>)}
