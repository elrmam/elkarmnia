...
function Shops() {
  ...
  useEffect(() => {
      ...
      }, [selectedCity, selectedOwner, sortOrder, Shops]);
    
    const uniqueCities = [...new Set(Shops.map(apt => apt.city))].filter(Boolean);
    const uniqueOwners = [...new Set(Shops.map(apt => apt.owner))].filter(Boolean);
