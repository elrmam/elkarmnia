...
function Apartments() {
  ...
  useEffect(() => {
    fetch("https://script.google.com/macros/s/AKfycbzKyerIdao69xRzwiyjSckHoSL64bC5HOyX6jcRKkNRPXBx5owfF31LPVR54_hK0JBPfg/exec")
      .then((res) => res.json())
      .then((data) => {
        setApartments(data);
  ...
  }, [selectedCity, selectedOwner, selectedType, sortOrder, apartments]);

  const uniqueCities = [...new Set(apartments.map(apt => apt.city))].filter(Boolean);
  const uniqueOwners = [...new Set(apartments.map(apt => apt.owner))].filter(Boolean);
  ...
  return (
    <div className="container py-4" dir="rtl">
      <h1 className="darkgray fw-bold rounded sectionehead my-5 p-3 text-center">الشقق</h1>
      ...
      <div className={`collapse ${showFilters ? "show" : ""} mb-5`}>
        <div className="card card-body bg-secondary border-0 shadow text-light p-4">
          <div className="row">
            ...
            <div className="col-md-3 mb-3 text-start">
              <label className="form-label fw-bold small">النوع :</label>
          ...
          <div className="text-center mt-3">
            <button className="btn btn-outline-warning btn-sm fw-bold px-4 text-black" onClick={() => {
              setSelectedCity(""); setSelectedOwner(""); setSelectedType(""); setSortOrder("");
            }}>إعادة الضبط  </button>
          </div>
        </div>
      </div>
        ...
        {filteredApartments.map((apt, index) => (
          <div key={index} className="col-lg-6 col-md-12 mb-4">
            <div className="card apartment-card shadow-sm border-0">
              ...
              {/* إعادة كل البيانات الأصلية هنا */}
              <div className="apartment-body bg-secondary">
                <ul className="list-unstyled p-3">
                  {apt.category && (<li className="yellow fs-4">{apt.category}</li>)}
                  ...
                  {apt.location && (<li className="yellow fs-4"> <strong className="text-light fs-5">اللوكيشن :</strong>{" "} <a href={apt.location} target="_blank" rel="noreferrer" className="text-info"> اضغط هنا </a> </li>)}
