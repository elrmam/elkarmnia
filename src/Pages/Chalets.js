...
function Chalets() {
  ...
  useEffect(() => {
    fetch("https://script.google.com/macros/s/AKfycbwxTSXQScUNANe_9UKDKaxvHXn8YwtBV1ihm-R9jqBRyV38ZDUmFTCFSP1zhYdqcnf5/exec")
      .then((res) => res.json())
      .then((data) => {
        setChalets(data);
        setFilteredChalets(data);
        setLoading(false);
      })
   ...
   useEffect(() => {
     ...
     if (sortOrder) {
      ...
      setFilteredChalets(result);
    }, [selectedCity, selectedOwner, sortOrder, Chalets]);
  
    const uniqueCities = [...new Set(Chalets.map(apt => apt.city))].filter(Boolean);
    const uniqueOwners = [...new Set(Chalets.map(apt => apt.owner))].filter(Boolean);
  ...
  <div className="card apartment-card shadow-sm border-0">
    ...
    <div className="apartment-body bg-secondary">
      <ul>
        ...
        {apt.number && (<li className="yellow fs-4"><strong className="text-light fs-5">رقم الشاليه :</strong> {apt.number}</li>)}
