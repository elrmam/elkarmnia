...
function Offices() {
  ...
  const [filteredChalets, setFilteredOffices] = useState([]);
  ...
  useEffect(() => {
    fetch("https://script.google.com/macros/s/AKfycbzuLlQKyMtTigPUcNl6EG-JrxnXcOqRaKBVsnDJmStqiNiURP_rw28f0hDsIz-7oL_d/exec")
      .then((res) => res.json())
      .then((data) => {
        setOffices(data);
        setLoading(false); 
  ...
  useEffect(() => {
      ...
      }, [selectedCity, selectedOwner, sortOrder, Offices]);
    
      const uniqueCities = [...new Set(Offices.map(apt => apt.city))].filter(Boolean);
      const uniqueOwners = [...new Set(Offices.map(apt => apt.owner))].filter(Boolean);
